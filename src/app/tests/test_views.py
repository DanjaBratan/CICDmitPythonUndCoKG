...
def test_add_note(app, client, test_user):
    ...
    with app.app_context(): # Überprüfe, ob die Notiz erfolgreich hinzugefügt wurde
        added_note = Note.query.filter_by(data='Testnotiz', user_id=test_user.id).first()
...
def test_delete_note(app, client, test_user):
    ...
    assert response.status_code == 200
    assert not Note.query.get(note.id) # Überprüfen, ob die Notiz aus der Datenbank entfernt wurde
