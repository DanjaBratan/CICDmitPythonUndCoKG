...
def home():
    if request.method == "POST":  # Überprüft, ob das Formular abgesendet wurde
        ...
        if len(note) < 1:
            ...
            )  # Erstellt eine neue Notiz mit den angegebenen Daten
...
def delete_user(user_id):
    user = User.query.get(
        user_id
    )  # Ruft den Benutzer aus der Datenbank anhand seiner ID ab
    if user:
        if (
            ...
            flash(
                "Du hast nicht die Berechtigung, den Account zu löschen!.",
                category="error",
            )
    else:
...
def updateUserEmail(user_id):
    ...
    if user:
        if (
            ...
            flash(
                "Du hast nicht die Berechtigung, diese Mail zu ändern!",
                category="error",
            )
    else:
        flash("User nicht gefunden!.", category="error")
